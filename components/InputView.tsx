  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-6 relative overflow-hidden bg-black">
      ...
      <div className="absolute inset-0 z-0">
        <div className="absolute top-0 left-0 w-full h-full bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-gray-800 via-black to-black opacity-50"></div>
        <motion.div 
          className="absolute top-1/4 left-1/4 w-96 h-96 bg-purple-900/30 rounded-full blur-[100px]"
          animate={{ 
            x: [0, 50, -50, 0],
            y: [0, -50, 50, 0],
            scale: [1, 1.2, 0.9, 1]
        ...
        />
        <motion.div 
          className="absolute bottom-1/4 right-1/4 w-96 h-96 bg-blue-900/30 rounded-full blur-[100px]"
          animate={{ 
            x: [0, -30, 30, 0],
            y: [0, 30, -30, 0],
            scale: [1, 1.1, 0.9, 1]
      ...
      >
        <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/5 border border-white/10 mb-8">
          ...
          <span className="text-sm text-gray-300 tracking-wide uppercase">Powered by Gemini 2.5 Flash</span>
        </div>
        ...
        <p className="text-lg text-gray-400 mb-12 font-light max-w-md mx-auto">
        ...
        <form onSubmit={handleSubmit} className="relative group">
          ...
          <div className="relative flex items-center bg-black border border-white/20 rounded-full p-2 focus-within:border-white/50 transition-colors">
            <input
              type="text"
              value={topic}
              onChange={(e) => setTopic(e.target.value)}
              placeholder="e.g. The Colonization of Mars, Premium Coffee..."
              className="flex-1 bg-transparent border-none outline-none text-white px-6 py-4 text-lg placeholder-gray-600"
              autoFocus
            />
            <button 
              type="submit"
        ...
        <div className="mt-12 flex flex-wrap justify-center gap-3">
          {["Space Travel", "Sustainable Fashion", "Cyberpunk City", "Zen Gardens"].map((suggestion) => (
            <button
              ...
              className="px-4 py-2 rounded-full text-sm bg-white/5 hover:bg-white/10 border border-white/5 transition-colors text-gray-400 hover:text-white"
            >
              {suggestion}
            </button>
          ))}
